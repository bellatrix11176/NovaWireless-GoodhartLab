...
def main() -> int:
    ...
    ap.add_argument("--usage_snapshot_date", type=str, default="2025-12-31")
    args = ap.parse_args()
    ...
    # ---------------------------------------------------------------------------
    # Auto-copy generated files to data/ so call-gen can find them
    # ---------------------------------------------------------------------------
    import shutil
    data_dir = repo / "data"
    ...
    print(f"\n  Copying generated files to data/ ...")
    copied = []
    for fname in files_to_copy:
        src_file = repo / "output" / fname
        ...
        if src_file.exists():
            ...
            print(f"    [--] {fname} not found in output/ — skipping")

    print(f"\n{'='*60}")
    print(f"  ALL STEPS COMPLETE")
    print(f"  Outputs in:  {repo / 'output'}")
    print(f"  Copied {len(copied)} files → {data_dir}")
    print(f"{'='*60}\n")
    return 0
