...
TRUE_RESOLUTION_PROBS = {
    "clean":               0.84,   # governance — matches scenario_router.py (hard but resolvable)
    "unresolvable_clean":  0.10,
    "gamed_metric":        0.20,   # burnout bleed — governance value
...
def compute_outcome_flags(rng, scenario, friction_tier, rep_state):
    ...
    burnout = rep_state.get("rep_burnout_level",     rep_state.get("burnout_level", 0.0))
    skill   = rep_state.get("rep_policy_skill",      rep_state.get("policy_skill",  0.5))
    misinfo = rep_state.get("misinformation_risk",   0.10)

    # incorrect_info_given — tenure/burnout driven, scenario-weighted
    misinfo_scenario_mult = {
        "clean":               1.0,
        "unresolvable_clean":  0.8,
        "activation_clean":    1.3,
        "activation_failed":   1.2,
        "line_add_legitimate": 1.2,
        "gamed_metric":        1.1,
    }.get(scenario, 1.0)
    # Legacy fraud scenarios — low misinfo, rep knew what they were doing
    for fs in ("fraud_store_promo", "fraud_line_add", "fraud_hic_exchange", "fraud_care_promo"):
        if scenario == fs:
            misinfo_scenario_mult = 0.4
    p_misinfo = min(misinfo * misinfo_scenario_mult, 0.95)
    incorrect_info_given = rng.random() < p_misinfo
    ...
    # incorrect_info degrades true resolution
    ...
    true_res = rng.random() < p_true

    # churned — tied to true resolution, NOT proxy
    BASE_CHURN_PROB = {
        "clean":               0.04,
        ...
        "gamed_metric":        0.08,
        "fraud_store_promo":   0.18,
        "fraud_line_add":      0.22,
        "fraud_hic_exchange":  0.20,
        "fraud_care_promo":    0.15,
    }
    p_churn = BASE_CHURN_PROB.get(scenario, 0.05)
    if not true_res:
        p_churn = min(p_churn * 2.2, 0.60)
    if incorrect_info_given:
        p_churn = min(p_churn + 0.06, 0.70)
...
def main() -> int:
    ...
    for _, row in df.iterrows():
        ...
        rep_state = {
            ...
            # Tenure and misinformation — from call record columns added in generate_calls.py
            "misinformation_risk":   float(row.get("agent_misinformation_risk") or 0.10),
            "tenure_band":           str(row.get("agent_tenure_band")           or "senior"),
            "burnout_level":         float(row.get("rep_burnout_level")         or 0.3),
            "policy_skill":          float(row.get("rep_policy_skill")          or 0.5),
        }
